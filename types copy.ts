...
export type NumericModelField<
	...
	P extends string = "",
> = {
	[K in keyof ModelType<D, T, H, P>]: ModelType<D, T, H, P>[K] extends number
		? K
		: never;
}[keyof ModelType<D, T, H, P>];

export type StringModelField<
	D extends DatabaseName,
	T extends ModelName,
	H extends boolean = false,
	P extends string = "",
> = {
	[K in keyof ModelType<D, T, H, P>]: ModelType<D, T, H, P>[K] extends string
		? K
		: never;
}[keyof ModelType<D, T, H, P>];
...
export type BooleanModelField<
	...
	P extends string = "",
> = {
	[K in keyof ModelType<D, T, H, P>]: ModelType<D, T, H, P>[K] extends boolean
		? K
		: never;
}[keyof ModelType<D, T, H, P>];

export type DateModelField<
	D extends DatabaseName,
	T extends ModelName,
	H extends boolean = false,
	P extends string = "",
> = {
	[K in keyof ModelType<D, T, H, P>]: ModelType<D, T, H, P>[K] extends Date
		? K
		: never;
}[keyof ModelType<D, T, H, P>];

export type ArrayModelField<
	D extends DatabaseName,
	T extends ModelName,
	H extends boolean = false,
	P extends string = "",
> = {
	[K in keyof ModelType<D, T, H, P>]: ModelType<D, T, H, P>[K] extends any[]
		? K
		: never;
}[keyof ModelType<D, T, H, P>];
...
export type ReferenceModelField<
	...
	P extends string = "",
> = {
	[K in keyof ModelType<D, T, H, P>]: ModelType<
		D,
		T,
		H,
		P
	>[K] extends ReferenceFieldType
		? K
		: never;
}[keyof ModelType<D, T, H, P>];
...
export type NumericValue<
	...
	P extends string = "",
> =
	| NumericModelField<D, T, H, P>
	| ReferenceModelField<D, T, H, P>
	| number
	| QueryFunction<D, T, H, P>;
...
export type BooleanValue<
	...
	H extends boolean = false,
	P extends string = "",
> = BooleanModelField<D, T, H, P> | boolean | QueryFunction<D, T, H, P>;
...
> =
	| NumericModelField<D, T, H, P>
	| StringModelField<D, T, H, P>
	| BooleanModelField<D, T, H, P>
	| DateModelField<D, T, H, P>
	| ReferenceModelField<D, T, H, P>
	| ArrayModelField<D, T, H, P>
	| QueryFunction<D, T, H, P>;
...
export type DateValue<
	...
	H extends boolean = false,
	P extends string = "",
> = DateModelField<D, T, H, P> | Date | QueryFunction<D, T, H, P>;
...
export type ArrayValue<
	...
	H extends boolean = false,
	P extends string = "",
> = ArrayModelField<D, T, H, P> | any[] | QueryFunction<D, T, H, P>;
...
export type FieldCondition<
	...
	M = ModelType<D, T, H, P>,
> = {
	[K in keyof M]?: M[K] | QueryFunction<D, T, H, P, M>;
};
...
export type WhereCondition<
	...
	P extends string = "",
	M = ModelType<D, T, H, P>,
> = FieldCondition<D, T, H, P, M> & QueryFunction<D, T, H, P, M>;
...
> = {
	 ...
	 */
	$eq?: [leftOperand: AnyValue<D, T, H, P>, rightOperand: AnyValue<D, T, H, P>];
	/**
	 * Checks not-equality of two values
	 */
	$neq?: [
		leftOperand: AnyValue<D, T, H, P>,
		rightOperand: AnyValue<D, T, H, P>,
	];
	/**
	 * Checks whether the first value is less than the second value
	 */
	$lt?: [
		firstValue: NumericValue<D, T, H, P>,
		secondValue: NumericValue<D, T, H, P>,
	];
	/**
	 ...
	 */
	$in?: [value: AnyValue<D, T, H, P>, arrayOfValues: ArrayValue<D, T>];
	/**
	 * Checks whether the value is not in an array
	 */
	$nin?: [value: AnyValue<D, T, H, P>, arrayOfValues: ArrayValue<D, T>];
	/**
	 * Performs logical and
	 */
	$and?: BooleanValue<D, T, H, P>[];
	/**
	 * Performs logical or
	 */
	$or?: BooleanValue<D, T, H, P>[];
	/**
	 * Performs logical not
	 */
	$not?: BooleanValue<D, T, H, P> | [value: BooleanValue<D, T, H, P>];
	/**
	 * Checks if the value exists or not
	 */
	$exists?: AnyValue<D, T, H, P> | [value: AnyValue<D, T, H, P>];
	/**
	 * Returns the absolute value of a number.
	 */
	$abs?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Adds numbers together
	 */
	$add?: NumericValue<D, T, H, P>[];
	/**
	 * Returns the result of dividing the first number by the second
	 */
	$divide?: [
		dividend: NumericValue<D, T, H, P>,
		divisor: NumericValue<D, T, H, P>,
	];
	/**
	 * Returns the smallest integer greater than or equal to the specified number
	 */
	$ceil?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Returns the largest integer less than or equal to the specified number
	 */
	$floor?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 ...
	 */
	$multiply?: NumericValue<D, T, H, P>[];
	/**
	 * Rounds a number to a whole integer or to a specified decimal place
	 */
	$round?: [
		number: NumericValue<D, T, H, P>,
		decimalPlaces: NumericValue<D, T, H, P>,
	];
	/**
	 * Calculates the square root of a positive number and returns the result as a decimal
	 */
	$sqrt?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Subtracts two numbers to return the difference
	 */
	$subtract?: [
		number1: NumericValue<D, T, H, P>,
		number2: NumericValue<D, T, H, P>,
	];
	/**
	 * Concatenates strings and returns the concatenated string
	 */
	$concat?: StringValue<D, T, H, P>[];
	/**
	 * Checks whether a string starts with the characters of a specified string, returning true or false as appropriate
	 */
	$startsWith?: [
		mainText: StringValue<D, T, H, P>,
		searchText: StringValue<D, T, H, P>,
	];
	/**
	 ...
	 */
	$left?: [
		text: StringValue<D, T, H, P>,
		characterCount: NumericValue<D, T, H, P>,
	];
	/**
	 ...
	 */
	$length?: StringValue<D, T, H, P> | [text: StringValue<D, T, H, P>];
	/**
	...
	$charindex?: [
		mainText: StringValue<D, T, H, P>,
		searchText: NumericValue<D, T, H, P>,
		startingIndex: NumericValue<D, T, H, P>,
	];
	/**
	 ...
	 */
	$exp?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Calculates the natural logarithm of a number and returns the result as a decimal number
	 */
	$ln?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Calculates the log of a number in the specified base and returns the result as a double
	 */
	$log?: [number: NumericValue<D, T, H, P>, base: NumericValue<D, T, H, P>];
	/**
	 * Calculates the log base 10 of a number and returns the result as a decimal number
	 */
	$log10?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 ...
	 */
	$pow?: [number: NumericValue<D, T, H, P>, exponent: NumericValue<D, T, H, P>];
	/**
	 * Returns the sine of a value that is measured in radians
	 */
	$sin?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Returns the cosine of a value that is measured in radians
	 */
	$cos?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Returns the tangent of a value that is measured in radians
	 */
	$tan?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Returns the hyperbolic sine of a value that is measured in radians
	 */
	$sinh?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Returns the hyperbolic cosine of a value that is measured in radians
	 */
	$cosh?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Returns the hyperbolic tangent of a value that is measured in radians
	 */
	$tanh?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Returns the inverse sine (arcsine) of a number in radians, in the range -Pi/2 to Pi/2
	 */
	$asin?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Returns the inverse cosine (arccosine) of a number, in radians in the range 0 to Pi
	 */
	$acos?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Returns the inverse tangent (arctangent) of a value in radians, in the range -Pi/2 to Pi/2
	 */
	$atan?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Returns the inverse tangent (arc tangent) of y / x
	 */
	$atan2?: [y: NumericValue<D, T, H, P>, x: NumericValue<D, T, H, P>];
	/**
	 * Returns the inverse hyperbolic sine (hyperbolic arcsine) of a value
	 */
	$asinh?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Returns the inverse hyperbolic cosine (hyperbolic arc cosine) of a value
	 */
	$acosh?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Returns the inverse hyperbolic tangent (hyperbolic arctangent) of a value in radians
	 */
	$atanh?: NumericValue<D, T, H, P> | [value: NumericValue<D, T, H, P>];
	/**
	 * Converts an input value measured in degrees to radians
	 */
	$radians?: NumericValue<D, T, H, P> | [degress: NumericValue<D, T, H, P>];
	/**
	 * Converts an input value measured in radians to degrees
	 */
	$degrees?: NumericValue<D, T, H, P> | [radians: NumericValue<D, T, H, P>];
	/**
	 * Adds a period of time to the input date & time value and returns the resulting date & time value
	 */
	$dateAdd?: [
		date: DateValue<D, T, H, P>,
		duration: NumericValue<D, T, H, P>,
		unitOfMeasure:
			| "year"
			| "quarter"
			| "week"
	 ...
	 */
	$dateDiff?: [
		startDate: DateValue<D, T, H, P>,
		endDate: DateValue<D, T, H, P>,
		unitOfMeasure:
			| "year"
			| "quarter"
			| "week"
	 ...
	 */
	$hour?: DateValue<D, T, H, P> | [date: DateValue<D, T, H, P>];
	/**
	 * Returns the minute part of a date as an integer between 0 and 59
	 */
	$minute?: DateValue<D, T, H, P> | [date: DateValue<D, T, H, P>];
	/**
	 * Returns the second part of a date as a number between 0 and 59
	 */
	$second?: DateValue<D, T, H, P> | [date: DateValue<D, T, H, P>];
	/**
	 * Returns the year part of a date
	 */
	$year?: DateValue<D, T, H, P> | [date: DateValue<D, T, H, P>];
	/**
	 * Returns the month of a date as a number between 1 and 12
	 */
	$month?: DateValue<D, T, H, P> | [date: DateValue<D, T, H, P>];
	/**
	 * Returns the day of the month for a date as a number between 1 and 31
	 */
	$dayOfMonth?: DateValue<D, T, H, P> | [date: DateValue<D, T, H, P>];
	/**
	 * Returns the day of the week for a date as a number between 1 (Sunday) and 7 (Saturday)
	 */
	$dayOfWeek?: DateValue<D, T, H, P> | [date: DateValue<D, T, H, P>];
	/**
	 * Returns the day of the year for a date as a number between 1 and 366
	 */
	$dayOfYear?: DateValue<D, T, H, P> | [date: DateValue<D, T, H, P>];
	/**
	 ...
	 */
	$strToDate?: StringValue<D, T, H, P> | [dateString: StringValue<D, T, H, P>];
	/**
	 * Converts the input value to a decimal
	 */
	$toDecimal?: AnyValue<D, T, H, P> | [value: AnyValue<D, T, H, P>];
	/**
	 * Converts the input value to a boolean
	 */
	$toBoolean?: AnyValue<D, T, H, P> | [value: AnyValue<D, T, H, P>];
	/**
	 * Converts the input value to an integer
	 */
	$toInteger?: AnyValue<D, T, H, P> | [value: AnyValue<D, T, H, P>];
	/**
	 * Converts the input value to a date
	 */
	$toDate?: AnyValue<D, T, H, P> | [value: AnyValue<D, T, H, P>];
	/**
	 * Converts the input value to string
	 */
	$toString?: AnyValue<D, T, H, P> | [value: AnyValue<D, T, H, P>];
	/**
	 ...
	 */
	$toObjectId?: AnyValue<D, T, H, P> | [value: AnyValue<D, T, H, P>];
	/**
	 * Calculates the distance between two geo points in meters
	 */
	$distance?: [point1: AnyValue<D, T, H, P>, point2: AnyValue<D, T, H, P>];
	/**
	 * Constructs and returns a geo point value given the constituent longitude and latitude properties
	 */
	$point?: [
		longitude: NumericValue<D, T, H, P>,
		latitude: NumericValue<D, T, H, P>,
	];
};
...
> = {
	 ...
	 * Checks equality of two values
	 */
	$eq?: [identifier: AnyValue<D, T, H, P>, rightOperand: AnyValue<D, T, H, P>];
	/**
	 * Checks not-equality of two values
	 */
	$neq?: [
...
export type SortingOrder<
	...
	P extends string = "",
> = {
	[K in keyof ModelType<D, T, H, P>]?: SortDirection;
};
...
> = {
	 ...
	 * @type {string}
	 */
	where: WhereCondition<D, ModelList<D>, H, P>;
};
...
> =
	| ReferenceModelField<D, T, H, P>
	| Join<D, T, H, P>
	| (ReferenceModelField<D, T, H, P> | Join<D, T, H, P>)[];

export type UpdateOperation<
	D extends DatabaseName,
	T extends ModelName,
	K extends keyof ModelType<D, T, H, P>,
	H extends boolean = false,
	P extends string = "",
> =
	| { $set: ModelType<D, T, H, P>[K] }
	| { $unset: any }
	| { $inc: number }
	| { $mul: number }
	| { $min: number }
	| { $max: number }
	| { $push: any }
	| { $pull: PullQueryFunction<D, T, H, P> }
	| { $pop: any }
	| { $shift: any }
	| ModelType<D, T, H, P>[K];
...
export type UpdateDefinition<
	...
	P extends string = "",
> = {
	[K in keyof ModelType<D, T, H, P>]?: UpdateOperation<D, T, K, H, P>;
};
...
export type FindByIdArgs<D extends DatabaseName, T extends ModelName> = {
	 ...
	 * @type {string}
	 */
	select?: (keyof ModelType<D, T, false>)[];
	 ...
	 * @type {string}
	 */
	omit?: (keyof ModelType<D, T, false>)[];

	/**
	 * The join(s) to make (left outer join) while getting the record from the database
	 * @type {string}
	 */
	join?: JoinDefinition<D, T, false>;
...
export type AggregateArgs<D extends DatabaseName, T extends ModelName> = {
	 ...
	 * @type {string}
	 */
	where: WhereCondition<D, T, false>;
