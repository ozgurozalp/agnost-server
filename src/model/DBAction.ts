...
export class DBAction {
	...
	/**
	 * Sets the baseModel part of the db action definition
	 * @param {string} searchText The search string
	 */
	setBaseModel(baseModel: string) {
		...
		const model = this.model.getDb().model(baseModel);
		if (!model)
			throw new ClientError(
				"model_not_found",
				`The 'baseModel' parameter should match to a model in the database. There is no model named '${baseModel}' in datababase '${this.model
					.getDb()
					.getName()}'`
			);

		this.definition.baseModel = baseModel;
	}
	...
	/**
	 * Sets whether to return the count info of the matching records
	 * @param {boolean} countInfo The cound info flag
	 */
	setReturnCount(returnCount: boolean) {
		if (returnCount === null || returnCount === undefined) return;

		if (!isBoolean(returnCount))
			throw new ClientError(
				"invalid_value",
				`Return count can be boolean true or false`
			);

		this.definition.returnCount = returnCount;
	}
	...
	setSearchField(searchField: string) {
		...
		const fieldObj = this.getFieldObject(searchField, null);
		if (!fieldObj) {
			throw new ClientError(
				"invalid_field",
				`'${searchField}' is not a valid field that can be used to perform full-text search.`
			);
		} else if (!fieldObj.field.isSearchable()) {
			throw new ClientError(
				"invalid_field",
				`'${searchField}' is not a valid searchable field that has a full-text index.`
			);
		}

		this.definition.searchField = fieldObj;
	}
	...
	setSelect(select: string[] | null | undefined, join: any | null | undefined) {
		...
		if (errorList.length > 0) {
			throw new ClientError(
				"invalid_field",
				`Select option needs to specify the names of valid fields of the base model or fields of the joined/lookup models. The following fields cannot be specified in select option '${errorList.join(
					", "
				)}'`
			);
		}
	...
	/**
	 * Checks if there are duplicate alias entries in lookup and join definitions
	 */
	checkJoinAndLookupDuplicates() {
		if (!this.definition.lookup || !this.definition.join) return;

		for (const lookupEntry of this.definition.lookup) {
			for (const joinEntry of this.definition.join) {
				if (lookupEntry.as.toLowerCase() === joinEntry.as.toLowerCase()) {
					throw new ClientError(
						"invalid_join_or_lookup",
						`Not a valid join/lookup definition. The alias '${lookupEntry.as}' has been used in both lookup and join definitions. The alias names need to be unique across lookup and join definitions.`
					);
				}
			}
		}
	...
	/**
	 * Sets the lookup part of the db action definition
	 * @param {JoinDefinition} join The join definition
	 */
	setLookup(join: any | null | undefined) {
		if (!join) return;
		...
		if (typeof join === "string")
			this.processStringBasedJoin(join, join, finalJoin, "lookup");
		else if (typeof join === "object" && !Array.isArray(join))
			this.processObjectBasedJoin(join, join, finalJoin, "lookup");
		else if (Array.isArray(join)) {
			for (const entry of join) {
				if (typeof entry === "string")
					this.processStringBasedJoin(entry, join, finalJoin, "lookup");
				else if (typeof entry === "object" && !Array.isArray(entry))
					this.processObjectBasedJoin(entry, join, finalJoin, "lookup");
				else
					throw new ClientError(
						"invalid_join_or_lookup",
		...
		this.definition.lookup = finalJoin;
	}
	...
	setJoin(join: any | null | undefined) {
		...
		// Check to see if this is a single string definition
		if (typeof join === "string")
			this.processStringBasedJoin(join, join, finalJoin, "join");
		else if (typeof join === "object" && !Array.isArray(join))
			this.processObjectBasedJoin(join, join, finalJoin, "join");
		else if (Array.isArray(join)) {
			for (const entry of join) {
				if (typeof entry === "string")
					this.processStringBasedJoin(entry, join, finalJoin, "join");
				else if (typeof entry === "object" && !Array.isArray(entry))
					this.processObjectBasedJoin(entry, join, finalJoin, "join");
				else
					throw new ClientError(
						"invalid_join_or_lookup",
						`Not a valid join/lookup definition. The join/lookup array needs to include either reference field names as string or complex join/lookup definition as JSON object with 'as', 'from' and 'where' values.`
	 ...
	 * @param {string} type The join type either "join" or "lookup"
	 */
	processStringBasedJoin(
		refFieldName: string,
		join: any,
		joinList: any[],
		type: string
		...
		if (!fieldDef || fieldDef.field.getType() !== "reference")
			throw new ClientError(
				"invalid_join_or_lookup",
				`'${refFieldName}' is not a valid reference field to join/lookup. You can either join/lookup reference fields or define join/lookup queries.`
			);
		...
		if (
			joinList.find(
				(entry) => entry.as.toLowerCase() === refFieldName.toLowerCase()
			)
		)
			throw new ClientError(
				"invalid_join_or_lookup",
				`There is already a join/lookup definition with the alias '${refFieldName}'.`
			);
	...
	processObjectBasedJoin(
		...
		if (
			...
			throw new ClientError(
				"invalid_join_or_lookup",
				`Join/lookup from '${joinDef.from}' as '${joinDef.as}' is not a valid join/lookup definition. You can either join/lookup reference fields or define join/lookup queries.`
			);
		...
		if (
			joinList.find(
				(entry) => entry.as.toLowerCase() === joinDef.as.toLowerCase()
			)
		)
			throw new ClientError(
				"invalid_join_or_lookup",
				`There is already a join/lookup definition with the alias '${joinDef.as}'.`
			);

		// This part is only valid for lookup definitions
		if (type === "lookup") {
			fieldDef.skip = null;
			fieldDef.limit = null;
			fieldDef.sort = null;

			if (!(joinDef.skip === null || joinDef.skip === undefined)) {
				if (!isInteger(joinDef.skip) || joinDef.skip < 0)
					throw new ClientError(
						"invalid_value",
						`Skip count can be zero or positive integer in lookup definition with alias '${joinDef.as}'`
					);

				fieldDef.skip = joinDef.skip;
			}

			if (joinDef.limit) {
				if (!isPositiveInteger(joinDef.limit))
					throw new ClientError(
						"invalid_value",
						`Limit needs to be a positive integer value in lookup definition with alias '${joinDef.as}'`
					);

				fieldDef.limit = joinDef.limit;
			}

			if (joinDef.sort) {
				// Create the groupby model definition
				const action = new DBAction(fieldDef.joinModel);
				action.setSort(joinDef.sort, joinDef);
				fieldDef.sort = action.getSort();
			}
		}
	...
	processExpression(
		...
		if (funcDefinition) {
			...
			// The entry is neither a comparison operator, logical operator or function definition then throw an error
				...
				throw new ClientError(
					"invalid_expression",
					`There is no comparison operator, logical operator, function or model field named '${key}'. You can use predefined functions, base model fields or joined model fields in your query expressions.`
				);
		}
	}
	...
	setSort(sort: any | null | undefined, join: any | null | undefined) {
		...
		for (const fieldName of keys) {
			...
			if (!fieldObj) {
				throw new ClientError(
					"invalid_field",
					`'${fieldName}' is not a valid field that can be used to sort query results. Only base model and joined model fields can be used in sort definitions.`
				);
			}
	...
	async setUpdates(updates: object, join: any | null | undefined) {
		...
		for (const [fieldName, value] of Object.entries(updates)) {
			...
			if (value === null) {
				...
				(typeof value === "object" && fieldObj.field.getType() === "json")
			) {
	...
	async processUpdateInstruction(
		...
		if (keys.length > 0) {
			...
			if (
				!UpdateOperators.includes(opType2.toLowerCase()) &&
				(fieldObj.field.getType() === "json" ||
					fieldObj.field.getType() === "binary")
			) {
		...
		if (!UpdateOperators.includes(opType.toLowerCase())) {
			throw new ClientError(
				"invalid_update_instruction",
	...
	async processSetInstruction(
		...
		if (
			...
			fieldObj.field.getType() !== "json" &&
			fieldObj.field.getType() !== "binary" &&
			fieldObj.field.getType() !== "geo-point"
		) {
			throw new ClientError(
				"invalid_value",
	...
	processPopShiftInstruction(
		...
		if (this.model.getDb().getType() !== DBTYPE.MONGODB) {
			throw new ClientError(
				"invalid_update_instruction",
				`Update type '$${opType}' cannot be used in '${this.model
					.getDb()
					.getType()}' databases.`
			);
		}
	...
	processStringBasedGrouping(fieldName: string, join: any, groupList: any[]) {
		...
		if (fieldName.includes(".")) {
			...
			const asEntry = entries[entries.length - 1];
			throw new ClientError(
				"invalid_grouping_entry",
				`You are trying to group records by joined model '${fieldName}' using the dot notation (e.g., modelname.fieldname) which is not allowed. Instead try grouping using {as: '${asEntry}' , expression: '${fieldName}'}`
			);
		}
	...
	processObjectBasedGrouping(groupDef: any, join: any, groupList: any[]) {
		...
		if (!isObject(groupDef.expression) && !isString(groupDef.expression))
			throw new ClientError(
				"invalid_grouping_entry",
		...
		if (isObject(groupDef.expression)) {
			expression = this.processWhereCondition(
				groupDef.expression,
				join,
				ConditionType.QUERY
			);
	...
	setComputations(
		...
		for (const comp of compList) {
			...
			const operator = keys[0].toLowerCase();
