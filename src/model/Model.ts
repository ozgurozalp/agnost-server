...
export class Model<D extends DatabaseName, T extends ModelList<D>> {
	 ...
	 *
	 * @param {ModelTypeHierarchy<D, T>} data The data that will be inserted into the database
	 ...
	 */
	async createOne(data: ModelTypeHierarchy<D, T>): Promise<object> {
	 ...
	 *
	 * @param {ModelTypeHierarchy<D, T>[]} data The list of records that will be inserted into the database
	 ...
	 */
	async createMany(data: ModelTypeHierarchy<D, T>[]): Promise<CountInfo> {
	 ...
	 *   - select?: Array of fields to include on the returned record, can include fields of base model, joined and looked up models. If not provided, checks the `omit` list if `omit` is also not provided then all fields will be returned. You can specifiy either `select` or `omit` but not both.
	 ...
	 *   - returnCount?: Specifies whether to return the count information or not. If set to true, returns an object which includes count information and list of matched objects.
	 * @returns Returns the matching records otherwise an empty array. If `returnCountInfo=true`, returns an object which includes count information and list of matched objects.
	...
	/**
	 * Deletes the record identified by the id and returns the deleted document count.
	 *
	 ...
	 *   - skip?: Number of records to skip.
	 *   - limit?: Max number of objects to return.
	 *   - baseModel: The main model where this subquery will be used. This is a required field tha needs to be specified.
	 * @returns Returns the matching records otherwise an empty array
	 * @throws Throws an exception if the database records cannot be retrieved
	 */
	async getSQLSubQuery(args: GetSQLSubQueryArgs<D, T>): Promise<string> {
		return await this.modelBase.getSQLSubQuery(args);
	}
}
