...
export class Field {
  ...
  /**
   * Returns true if the database is an SQL DB
   * @returns True if SQL database, otherwise false
   */
  isSQLDB(): boolean {
  ...
  hasDefaultValue(): boolean {
    return (
      this.meta.defaultValue !== null &&
      this.meta.defaultValue !== undefined &&
   ...
   */
  getLanguage(): string {
    if (this.getDBType() === "MySQL") return "utf8mb4_unicode_ci";
    else return "english";
  }
  ...
  async prepareForCrete(
    ...
    if (rawValue == null || rawValue === undefined) {
      ...
      // If it is not a required field, simply we do not create any value for this field and ignore it
        ...
        await this.setValue({}, processedData, response, true, index);
      } else if (this.isSQLDB() || (!this.isSQLDB() && rawValue === null)) {
        await this.setValue(null, processedData, response, true, index);
      } else return;
    } else {
